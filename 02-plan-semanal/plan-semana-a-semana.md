# Plan de trabajo semana a semana — AndyBites (reposicionamiento)

Actualizado: 15 de septiembre de 2026 (semana 5 del curso). El focus 2 se hace el domingo 20 de septiembre. Las semanas 5 y 6 salen directamente de lo que pidió la profesora en clase; de la semana 7 en adelante es una propuesta del asesor basada en la secuencia típica del curso (prototipo, validación, plan comercial, lanzamiento). Hay que confirmarla con el sílabo o con la profesora y ajustar fechas.

Equipo: Diana Ayoso (DA), Carlos Inga (CI), Angie Blas (AB), Patricia Cárdenas (PC), Adela Robles (AR). Los responsables son una sugerencia; cámbienlos según disponibilidad.

## Regla de oro de la profesora
Primero se prueba el producto, después se pregunta el precio. El público define el precio del pack de 6, no el equipo. Y hay que validar cinco cosas: producto, empaque, nombre, calidad y precio.

---

## Semana 5 (15 al 20 de septiembre) — Producir, costear y hacer el focus 2 el domingo 20
Objetivo: hacer el focus 2 el domingo 20 en el colegio de Patty (solo puede entrar ella) con producto fresco, empaque MVP y fichas listas, y llegar a la clase de la semana 6 con resultados para la presentación T1.

Reparto acordado el 16 de septiembre (5 bloques, uno por persona):

| Bloque | Responsable | Tareas | Entrega a |
|---|---|---|---|
| 1. Insumos, producción y focus | Patty | Comprar insumos anotando precios; producir el lote el sábado pesando cada unidad a 20 g con foto de la balanza; el domingo moderar y grabar el focus, recoger las fichas | Precios de insumos a Angie; video y fichas a Ade |
| 2. Guía del focus y elevator pitch | Ade | Preguntas y fichas del focus antes del domingo; subir el video a YouTube y pasar las respuestas en limpio; elevator pitch en video (guion, avatar con IA, edición) | Guía y fichas a Patty antes del domingo; link de YouTube y video del pitch a Diana |
| 3. Empaque, buyer personas y precios de mercado | Carlos | Bolsa o envoltura con sticker y 8-10 packs listos antes del domingo; 2 buyer personas; visita a 2-3 tiendas (naturista, Wong, Vivanda) anotando marca, gramaje y precio | Packs a Patty antes del domingo; benchmarks de precio a Angie; personas a Diana |
| 4. Costeo y plan de marketing | Angie | Ficha de costo unitario (lote de 5 y proyección a 10,000); con el precio del focus, estimación de ventas del próximo año, punto de equilibrio y estrategia de canales (mamás influencers, eventos infantiles, colegios, WhatsApp) | Costeo y plan a Diana |
| 5. Canvas, DIGESA y presentación T1 | Diana | Business model canvas con resultados del focus; lámina de la ruta regulatoria (DIGESA y octógonos); juntar todo en Canva para la presentación de 12-15 minutos | Presentación final al grupo |

Dependencias:
- Los bloques 2 y 3 entregan a Patty antes del domingo (guía, fichas y packs).
- Del focus salen precio, nombre y empaque: con eso Diana hace el canvas, Angie el punto de equilibrio y Carlos ajusta los buyer personas.
- Diana cierra al final con lo de todos.

Material de apoyo en este repositorio: guía del focus (03-entregables/05), checklist de empaque y costeo (03-entregables/06), borradores de buyer personas (01), canvas (02), pitch (03) y plan de marketing (04).

Guion sugerido para la exposición de 12-15 minutos:
1. Qué aprendimos en semanas 1-4 y por qué cambiamos de segmento (2 min).
2. Nuevo segmento: 3 arquetipos y 2 buyer personas (3 min).
3. Producto y empaque MVP: mostrar las muestras físicas y el pack de 6 (2 min).
4. Costeo: costo unitario del lote de 5 y qué cambia a 10,000 (2 min).
5. Canvas y elevator pitch (3 min).
6. Plan de marketing y ventas: canales prioritarios y calendario (2 min).
7. Resultados del focus 2: precio, nombre, empaque y qué ajustamos (1-2 min).

## Semana 6 (21 al 27 de septiembre) — Exponer con resultados del focus 2
| # | Tarea | Responsable | Entregable |
|---|---|---|---|
| 1 | Exposición de 12-15 minutos en clase. Llevar muestras, empaque MVP, ficha de costos, benchmarks y resultados del focus 2 | Todos | Presentación + retroalimentación de la profesora anotada |
| 2 | Informe del focus 2 (2-3 páginas): tabla resumen, rango de precio aceptable, decisión sobre nombre y empaque | A elegir | Informe |
| 3 | Ajustar canvas, buyer personas y plan de marketing con lo que dijo el público | Todos | Versión 2 de entregables |
| 4 | Si faltaron participantes o datos en el focus, hacer 2-3 entrevistas cortas de complemento con degustación | A elegir | Notas |

## Semana 7 (28 de septiembre al 4 de octubre) — Ajustar producto y definir precio [propuesta]
- Ajustes de receta según el focus (dulzor, humedad, tamaño) y nueva prueba de vida útil.
- Fijar precio de lanzamiento del pack de 6 usando el rango del focus y el costeo con margen.
- Diseño final del empaque MVP (versión 2) y contenido de la etiqueta.
- Abrir Instagram y WhatsApp Business de AndyBites; primer contenido: detrás de cámaras de la producción.

## Semana 8 (5 al 11 de octubre) — Piloto de venta 1: venta directa [propuesta]
- Primera venta real a 10-15 familias del focus y contactos (pack de 6 por WhatsApp). Registrar cada venta: quién, cuántos, precio, comentarios.
- Entregar 5-8 packs de cortesía a 2-3 mamás influencers pequeñas (micro, 3-20 mil seguidores) a cambio de reseña honesta.
- Encuesta corta de recompra a los 5 días.

## Semana 9 (12 al 18 de octubre) — Piloto de venta 2: evento o feria [propuesta]
- Participar en una feria saludable, feria de colegio o evento infantil (o montar un "carrito" en un cumpleaños). Medir muestras entregadas, ventas, precio aceptado.
- Cerrar la ruta regulatoria en papel: qué exige DIGESA, costo y tiempo de análisis de laboratorio, etiquetado y octógonos.

## Semana 10 (19 al 25 de octubre) — Canal institucional [propuesta]
- Reunión con 1-2 colegios privados (coordinadora de quiosco saludable o escuela de padres) y 1-2 tiendas naturistas. Llevar muestras y ficha comercial (precio al canal, margen, condiciones).
- Consolidar resultados de los pilotos y actualizar el plan de marketing y ventas.

## Semana 11 (26 de octubre al 1 de noviembre) — Plan financiero [propuesta]
- Proyección de ventas a 12 meses, punto de equilibrio, inversión inicial, flujo de caja simple.
- Escenario de 10,000 unidades: proveedores a escala, cocina o taller alquilado, personal.

## Semana 12 (2 al 8 de noviembre) — Marca y comunicación [propuesta]
- Identidad visual definitiva, manual básico de marca, calendario de contenidos del trimestre.
- Material de venta: catálogo de una página, script de WhatsApp, propuesta para colegios.

## Semana 13 (9 al 15 de noviembre) — Plan de lanzamiento [propuesta]
- Definir fecha y mecánica de lanzamiento (evento, campaña digital, preventa navideña).
- Preparar inventario y logística de entregas.

## Semana 14 (16 al 22 de noviembre) — Evento de lanzamiento o marcha blanca [propuesta]
- Ejecutar el lanzamiento y registrar resultados (unidades, ingresos, clientes nuevos, contenido generado).

## Semana 15 (23 al 29 de noviembre) — Informe final [propuesta]
- Redactar el informe integrador: investigación, producto, validación, canvas, marketing y ventas, finanzas, lanzamiento y aprendizajes.

## Semana 16 (30 de noviembre al 6 de diciembre) — Presentación final [propuesta]
- Exposición final con muestras, resultados de ventas y plan a 12 meses.

---

## Pendientes que dependen de la profesora (preguntar en la próxima clase)
1. ¿La exposición de la próxima semana es evaluada o es avance?
2. ¿Qué formato quiere para el canvas (Osterwalder clásico o Lean Canvas)?
3. ¿El plan de marketing y ventas debe incluir presupuesto y proyección financiera o eso va en otra semana?
4. ¿Hay fecha para el lanzamiento o marcha blanca real?
5. ¿Acepta prototipo sin registro sanitario para la prueba de producto (sí, es lo normal, pero confirmar)?

## Reglas de trabajo del equipo (propuesta)
- Todo lo que se produce se guarda en el Drive del curso y en este repositorio.
- Cada tarea tiene un responsable y una fecha; se revisa en una llamada corta de 20 minutos los domingos.
- Ningún número de precio se decide antes del focus 2.
